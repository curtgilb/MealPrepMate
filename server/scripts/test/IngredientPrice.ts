...
async function loadPriceHistory() {
  ...
  const insertedPriceHistory: Promise<unknown>[] = [];
  for (const ingredient of ingredients) {
    const createTask = limit(async () => {
      try {
        const history = generateFakeIngredientPriceHistory(ingredient.id);
        await createPriceHistory(history);
      } catch (error) {
        console.error("Error creating price history:", error);
      }
    });
    insertedPriceHistory.push(createTask);
  }
  await Promise.allSettled(insertedPriceHistory);
}
